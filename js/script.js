...

{
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const leafArea = document.querySelector('.leaf-area');
    const mask = document.querySelector('.js-mask');
    const nameTitle = document.querySelector('.name-ttl');
    const catchCopy = document.querySelector('.catch-copy');

    // FV用の要素が全部あるページだけ実行する
    if (line && leafArea && mask && nameTitle && catchCopy) {
      line.classList.add('wd-active');
      leafArea.classList.add('leaf-show');
      mask.setAttribute('data-animate', 'on');
      ...
      copyGroup.forEach(copyElement => {
        copyElement.classList.add('copy-show');
      });
    }
